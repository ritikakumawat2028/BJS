...
exports.campaignController = {
    ...
    create: async (req, res) => {
        try {
            ...
            if (req.user) {
                await (0, audit_1.logAdminAction)({ adminId: req.user.userId, action: 'CREATE', entity: 'Campaign', entityId: campaign.id, newValue: JSON.stringify(campaign), ipAddress: req.ip || '' });
            }
    ...
    update: async (req, res) => {
        try {
            ...
            if (req.user) {
                await (0, audit_1.logAdminAction)({ adminId: req.user.userId, action: 'UPDATE', entity: 'Campaign', entityId: campaign.id, previousValue: JSON.stringify(oldCampaign), newValue: JSON.stringify(campaign), ipAddress: req.ip || '' });
            }
            res.json({ success: true, data: campaign });
        }
        catch (error) {
    ...
    delete: async (req, res) => {
        try {
            ...
            if (req.user) {
                await (0, audit_1.logAdminAction)({ adminId: req.user.userId, action: 'DELETE', entity: 'Campaign', entityId: req.params.id, previousValue: JSON.stringify(campaign), ipAddress: req.ip || '' });
            }
