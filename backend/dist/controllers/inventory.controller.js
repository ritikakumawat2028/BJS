...
exports.getStockHistory = exports.adjustStock = exports.getInventoryList = exports.getInventoryStats = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
const error_1 = require("../middleware/error");
const notification_service_1 = require("../services/notification.service");
// 1. Get overall inventory stats
exports.getInventoryStats = (0, error_1.asyncHandler)(async (req, res) => {
...
exports.getInventoryList = (0, error_1.asyncHandler)(async (req, res) => {
...
exports.adjustStock = (0, error_1.asyncHandler)(async (req, res) => {
    const { id, isVariant, type, quantity, note } = req.body;
    // type: 'ADD', 'REMOVE', 'SET'
    if (!id || typeof isVariant === 'undefined' || !type || typeof quantity !== 'number') {
        throw (0, error_1.createError)('Invalid adjustment parameters', 400);
    }
    if (quantity < 0) {
        throw (0, error_1.createError)('Quantity must be positive', 400);
    }
    ...
    const result = await prisma_1.default.$transaction(async (tx) => {
        ...
        if (isVariant) {
            ...
            if (!variant)
                throw (0, error_1.createError)('Variant not found', 404);
        ...
        if (type === 'ADD') {
            ...
            newStock = currentStock + change;
            if (newStock < 0)
                throw (0, error_1.createError)('Cannot remove more stock than available', 400);
        }
        else if (type === 'SET') {
            ...
            throw (0, error_1.createError)('Invalid adjustment type', 400);
        }
        if (change === 0) {
    ...
    if (result.newStock !== undefined) {
        ...
        if (isVariant) {
            const variant = await prisma_1.default.productVariant.findUnique({ where: { id }, include: { product: true } });
            if (variant) {
            ...
            const p = await prisma_1.default.product.findUnique({ where: { id }, include: { inventory: true } });
            if (p) {
                productName = p.name;
                threshold = p.inventory?.lowStockThreshold || 5;
            }
        ...
        if (result.newStock <= threshold) {
            await notification_service_1.NotificationService.adminLowStock({
...
// 4. Get stock history
exports.getStockHistory = (0, error_1.asyncHandler)(async (req, res) => {
    const { id, isVariant } = req.query;
    const variantBool = isVariant === 'true';
    let transactions = [];
    if (variantBool) {
        transactions = await prisma_1.default.inventoryTransaction.findMany({
            where: { variantId: id },
    ...
    let sales = [];
    if (variantBool) {
        sales = await prisma_1.default.orderItem.findMany({
            where: { variantId: id },
