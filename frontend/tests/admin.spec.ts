...
test.describe('Admin End-to-End Flow', () => {
  test('Complete admin operations journey', async ({ page }) => {
    // 1. Admin Login
    ...
    await page.fill('input[name="name"]', `E2E Category ${timestamp}`);
    await page.click('button:has-text("Create Category")');
    await expect(page.locator(`text=E2E Category ${timestamp}`)).toBeVisible();

    // 3. Add Product
    await page.goto('http://localhost:5173/admin/products');
    await page.click('button:has-text("Add Product")');
    await page.fill('input[name="name"]', `E2E Luxury Soap ${timestamp}`);
    ...
    await page.selectOption('select[name="categoryId"]', { label: `E2E Category ${timestamp}` });
    
    await page.click('button:has-text("Publish Product")');
    await expect(page.locator(`text=E2E Luxury Soap ${timestamp}`)).toBeVisible();

    // 4. Update Stock (Inventory)
    await page.goto('http://localhost:5173/admin/inventory');
    // Search for the product
    await page.fill('input[placeholder*="Search"]', `SKU-${timestamp}`);
    ...
    await page.fill('input[name="name"]', `Diwali Sale ${timestamp}`);
    await page.fill('input[name="discount"]', '20');
    await page.fill('input[name="couponCode"]', `DIWALI${timestamp}`);
    await page.click('button:has-text("Create Campaign")');
    await expect(page.locator(`text=Diwali Sale ${timestamp}`)).toBeVisible();
